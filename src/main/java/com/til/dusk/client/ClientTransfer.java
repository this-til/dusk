            ...
            switch (clientParticleRegister.parsingMode) {
                case PAIR -> {
                    ...
                    for (Pos po : data.pos) {
                        ...
                        Extension.Data_2<Float, List<Particle>> data_2 = clientParticleRegister.run(Minecraft.getInstance().level, s, e, data.color, data.density);
                        if (data_2 != null) {
                            list.add(data_2);
                        }
                        s = null;
                ...
                case SINGLE -> {
                    for (Pos po : data.pos) {
                        Extension.Data_2<Float, List<Particle>> data_2 = clientParticleRegister.run(Minecraft.getInstance().level, po, data.color, data.density);
                        if (data_2 != null) {
                            list.add(data_2);
                        }
                    }
